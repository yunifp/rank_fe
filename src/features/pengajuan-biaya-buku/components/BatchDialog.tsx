...
import type { BatchItem } from "@/types/biayaBuku";
...
export function BatchDialog({
  ...
  const mutation = useMutation({
    mutationFn: (data: { kode_batch: string; ids: string }) =>
      biayaBukuService.batchBiayaBuku(data.kode_batch, data.ids),
    onSuccess: (res) => {
      if (res.success) {
        queryClient.invalidateQueries({ queryKey: ["pengajuan-biaya-buku"] });
        queryClient.invalidateQueries({
          queryKey: ["log-pengajuan", "biaya-buku"],
          exact: false,
        });
        onOpenChange(false);
        resetSelectedBatch();
    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-h-[90vh] overflow-y-auto font-inter">
          <DialogHeader>
            <DialogTitle>Batch Pengajuan Biaya Buku</DialogTitle>
            <DialogDescription>
