...
const FormAjukan = () => {
  ...
  const totalBiayaBuku =
    (statistikMahasiswa?.total_mahasiswa_aktif ?? 0) *
    (selectedPks?.biaya_buku_per_semester_per_mahasiswa ?? 0);
  ...
  const { data: getLockDataResponse, isLoading: isGetLockDataLoading } =
    useQuery({
      ...
      queryFn: () => {
        ...
        return biayaBukuService.getLockedDataPks(data);
      },
  ...
  useEffect(() => {
    ...
    setValue("jumlah_nominal", totalBiayaBuku);
  }, [
    selectedPks,
    statistikMahasiswa,
    semester,
                ...
                !keaktifanValid && (
                  <Alert className="w-full border-yellow-500 text-yellow-700 bg-yellow-50">
                    ...
                    <AlertDescription className="mt-1 leading-relaxed">
                      ...
                      periode {semester} belum tersedia. Proses pengajuan tidak
                      dapat dilanjutkan sebelum validasi keaktifan mahasiswa
                      diselesaikan. Pastikan data mahasiswa telah sesuai dan
                      ...
                      {!isGeneratingNominatif && watch("daftar_nominatif") && (
                        <Alert className="mt-2 border-green-200 bg-green-50 text-green-800">
                          ...
                          <AlertDescription className="text-green-700">
                            Daftar nominatif berhasil digenerate oleh sistem dan
                            siap dikirim bersama dokumen lainnya.
                          </AlertDescription>
                        </Alert>
                      )}
                    </div>
          ...
          <CardContent className="flex items-start gap-4 py-4">
            ...
            <div className="flex-1">
              <p className="font-semibold text-green-700 dark:text-green-300">
                Data Biaya Buku Sudah Diajukan
              </p>
              <p className="text-sm text-muted-foreground mt-1">
                Pengajuan biaya buku untuk PKS ini pada{" "}
                <b>Semester {semester}</b> telah diajukan ke verifikator dan
                sedang menunggu proses verifikasi.
              </p>
            </div>
          </CardContent>
        </Card>
