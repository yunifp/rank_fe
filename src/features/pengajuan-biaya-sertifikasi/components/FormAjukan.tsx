...
import {
  ...
  type ListSertifikasiWithMahasiswa,
...
const FormAjukan = () => {
  ...
  const { data: getLockDataResponse, isLoading: isGetLockDataLoading } =
    useQuery({
      queryKey: ["pks", "mahasiswa", "lock-data", selectedPksId, tahun],
      queryFn: () => {
        ...
        return biayaSertifikasiService.getLockedDataPks(data);
      },
      enabled: !!selectedPksId && !!tahun,
      retry: false,
      refetchOnWindowFocus: false,
      staleTime: STALE_TIME,
    });
  ...
  useEffect(() => {
    ...
    setValue("jumlah_nominal", totalBiayaSertifikasi);
  }, [selectedPks, statistikMahasiswa, tahun, totalBiayaSertifikasi, setValue]);
  ...
  const generateNominatifForForm = async () => {
    ...
    try {
      const blob = await biayaSertifikasiService.generateNominatif(
  ...
  const downloadSptjm = async () => {
    try {
      ...
      const blob = await biayaSertifikasiService.generateSptjm({
        id_trx_pks: parseInt(selectedPksId),
        nominal: 0,
  ...
  const downloadBuktiKwitansi = async () => {
    try {
      ...
      const blob = await biayaSertifikasiService.generateBuktiKwitansi({
        id_trx_pks: parseInt(selectedPksId),
        nominal: 0,
        total_mahasiswa_aktif: statistikMahasiswa?.total_mahasiswa_aktif ?? 0,
      });
  ...
  const {
    ...
    queryKey: ["get-list-bukti-pengeluaran", selectedPksId],
    queryFn: () => {
      return biayaSertifikasiService.getListBuktiPengeluaran(selectedPksId);
    },
    enabled: !!selectedPksId,
    retry: false,
    refetchOnWindowFocus: false,
  ...
  const listBuktiPengeluaran: ListSertifikasiWithMahasiswa[] =
          ...
          <CardContent className="flex items-start gap-4 py-4">
            ...
            <div className="flex-1">
              <p className="font-semibold text-green-700 dark:text-green-300">
                Data Biaya Sertifikasi Sudah Diajukan
              </p>
              <p className="text-sm text-muted-foreground mt-1">
                Pengajuan biaya sertifikasi untuk PKS ini pada periode{" "}
