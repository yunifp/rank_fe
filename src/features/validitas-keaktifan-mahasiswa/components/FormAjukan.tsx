...
import type { GetLockDataRequest, ITrxPks } from "@/types/pks";
import {
  ajukanValiditasKeaktifanMahasiswaSchema,
  type AjukanValiditasKeaktifanMahasiswaFormData,
...
const FormAjukan = () => {
  ...
  const form = useForm<AjukanValiditasKeaktifanMahasiswaFormData>({
    resolver: zodResolver(ajukanValiditasKeaktifanMahasiswaSchema),
  });
  ...
  const { data: getLockDataResponse, isLoading: isGetLockDataLoading } =
    useQuery({
      ...
      queryFn: () => {
        ...
        return validitasKeaktifanMahasiswaService.getLockedDataPks(data);
      },
      enabled: !!selectedPksId && !!bulan && !!tahun,
      retry: false,
      refetchOnWindowFocus: false,
      staleTime: STALE_TIME,
    });
  ...
  }, [selectedPks, statistikMahasiswa, bulan, tahun, setValue]);

  // Mengirim data ke API dan refresh cache
  const mutation = useMutation({
    mutationFn: (data: AjukanValiditasKeaktifanMahasiswaFormData) =>
      validitasKeaktifanMahasiswaService.create(data),
    onSuccess: (res) => {
      if (res.success) {
        queryClient.invalidateQueries({
          queryKey: ["validitas-keaktifan-mahasiswa"],
        });
        setSelectedPksId("");
  ...
  const onSubmit = (data: AjukanValiditasKeaktifanMahasiswaFormData) => {
    const isiPernyataan =
      `Dengan ini menyatakan bahwa sampai dengan Bulan ${bulan} sebanyak ${statistikMahasiswa?.total_mahasiswa_aktif} mahasiswa peserta Beasiswa ${selectedPks?.jenjang} Kelapa Sawit ${selectedPks?.lembaga_pendidikan} Tahun Ajaran ${selectedPks?.tahun_angkatan} aktif mengikuti kegiatan perkuliahan.`.trim();
                ...
                render={({ field, fieldState }) => (
                  <div className="space-y-2">
                    <div className="flex items-start gap-3">
                      ...
                      >
                        Dengan ini menyatakan bahwa sampai dengan Bulan {bulan}{" "}
                        sebanyak {statistikMahasiswa?.total_mahasiswa_aktif}{" "}
                        mahasiswa peserta Beasiswa {selectedPks.jenjang} Kelapa
                        Sawit {selectedPks.lembaga_pendidikan} Tahun Ajaran{" "}
                        {selectedPks.tahun_angkatan} aktif mengikuti kegiatan
                        perkuliahan.
                      </Label>
                    </div>
          ...
          <CardContent className="flex items-start gap-4 py-4">
            ...
            <div className="flex-1">
              <p className="font-semibold text-green-700 dark:text-green-300">
                Validitas Keaktifan Mahasiswa Bulanan Telah Divalidasi
              </p>
              <p className="text-sm text-muted-foreground mt-1">
                Validitas keaktifan mahasiswa untuk periode{" "}
                <b>
                  {bulan} {tahun}
                </b>{" "}
                telah berhasil divalidasi dan dinyatakan sah.
              </p>
            </div>
          </CardContent>
        </Card>
