...
import {
  pksSwakelolaCreateSchema,
  type ITrxPks,
  type PksSwakelolaCreateFormData,
} from "@/types/pks";
...
const KeuanganSwakelolaCreatePage = () => {
  useRedirectIfHasNotAccess("C");
  const queryClient = useQueryClient();
  const navigate = useNavigate();

  const form = useForm<PksSwakelolaCreateFormData>({
    resolver: zodResolver(pksSwakelolaCreateSchema),
    defaultValues: {
      jumlah_mahasiswa: 0,
      is_swakelola: "Y",
      pks_sebelumnya: [],
  ...
  const selectedPks = watch("pks_sebelumnya");
  ...
  const mutation = useMutation({
    mutationFn: (data: PksSwakelolaCreateFormData) =>
      pksService.createPksSwakelola(data),
    onSuccess: (res) => {
      if (res.success) {
        toast.success(res.message);
        queryClient.invalidateQueries({
          queryKey: ["pks-swakelola"],
          exact: false,
        });
        queryClient.invalidateQueries({
  ...
  const onSubmit = (data: PksSwakelolaCreateFormData) => {
...
export default KeuanganSwakelolaCreatePage;
