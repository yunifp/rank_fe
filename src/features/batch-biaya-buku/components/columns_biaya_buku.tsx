...
export const getColumnsBiayaBuku = (): ColumnDef<ITrxBiayaBukuPksWithPks>[] => [
  {
    id: "no",
    header: "No",
        ...
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="outline" size="icon" className="h-8 w-8" asChild>
                <a
                  href={row.original.file_pernyataan_keaktifan_mahasiswa!!}
                  target="_blank"
                  rel="noopener noreferrer"
                >
            ...
            <TooltipContent className="font-inter">
              <p>Surat Pernyataan Keaktifan Mahasiswa</p>
            </TooltipContent>
          </Tooltip>
        </TooltipProvider>
      </div>
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="flex flex-col items-start gap-2">
        ...
        {row.original.id_status == 2 && (
          <BadgeSubFlowLembagaPendidikan subFlow={row.original.sub_status!!} />
        )}
      </div>
    ),
  },
