...
import type { IMahasiswaPks, ITrxBiayaHidup } from "@/types/pks";
...
export const BiayaHidupTab = ({ data }: Props) => {
  ...
  const {
    ...
    queryFn: () => {
      return pksService.getBiayaHidupByMahasiswa(idTrxMahasiswa!!);
    },
    staleTime: STALE_TIME,
    enabled: idTrxMahasiswa !== undefined,
  });

  const dataBiayaHidup: ITrxBiayaHidup[] = response?.data ?? [];

  if (isLoading) {
    return (
      <TabsContent value="biaya-hidup" className="mt-0">
  ...
  return (
    <TabsContent value="biaya-hidup" className="mt-0 p-1">
      <div className="space-y-4">
        <div>
          <SectionHeader
            title="Riwayat Biaya Hidup"
            subtitle="Biaya Hidup yang telah diajukan"
            Icon={History}
          />
          <ModernTable
            headers={[
