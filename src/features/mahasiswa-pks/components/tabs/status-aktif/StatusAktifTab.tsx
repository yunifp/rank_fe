...
import { PerubahanStatusAktifDialog } from "../../../../../components/pks/PerubahanStatusAktifDialog";

interface Props {
  data?: IMahasiswaPks | null;
}

export const StatusAktifTab = ({ data: dataMahasiswa }: Props) => {
  const queryClient = useQueryClient();

  const [perubahanStatusAktifOpen, setPerubahanStatusAktifOpen] =
    useState(false);
  ...
  const mutation = useMutation({
    mutationFn: (data: StatusMahasiswaEditFormData) =>
      logPerubahanService.postPerubahanStatusAktif(
        dataMahasiswa!!.id,
  ...
  return (
    <>
      <TabsContent value="status-aktif" className="mt-0 p-1">
        <Button
          type="button"
          variant={"outline"}
          size={"sm"}
          onClick={() => {
            setPerubahanStatusAktifOpen(true);
          }}
          className="mb-4"
        >
      ...
      <LoadingDialog
        ...
        title="Merubah status mahasiswa"
      />

      <PerubahanStatusAktifDialog
        open={perubahanStatusAktifOpen}
        setOpen={setPerubahanStatusAktifOpen}
        idMahasiswa={dataMahasiswa?.id!!}
        hasPerubahan={dataMahasiswa?.has_perubahan_status_aktif === 1}
      />
    </>
  );
};
