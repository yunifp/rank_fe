...
import type { IMahasiswaPks, ITrxBiayaPendidikan } from "@/types/pks";
...
export const BiayaPendidikanTab = ({ data }: Props) => {
  ...
  const {
    ...
    queryFn: () => {
      return pksService.getBiayaPendidikanByMahasiswa(idTrxMahasiswa!!);
    },
    staleTime: STALE_TIME,
    enabled: idTrxMahasiswa !== undefined,
  });

  const dataBiayaPendidikan: ITrxBiayaPendidikan[] = response?.data ?? [];

  if (isLoading) {
    return (
      <TabsContent value="biaya-pendidikan" className="mt-0">
        ...
        <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
          <div className="text-red-600 mb-2">
            ...
            <p className="font-semibold text-lg">
              Gagal Memuat Data Biaya Pendidikan
            </p>
  ...
  const tableRows = dataBiayaPendidikan.map((item) => [
    item.semester ?? "-",
  ...
  return (
    <TabsContent value="biaya-pendidikan" className="mt-0 p-1">
      <div className="space-y-4">
        <div>
          <SectionHeader
            title="Riwayat Biaya Pendidikan"
            subtitle="Biaya Pendidikan yang telah diajukan"
            Icon={History}
          />
          <ModernTable
            headers={[
