...
import type { IMahasiswaPks, ITrxBiayaSertifikasi } from "@/types/pks";
...
export const BiayaSertifikasiTab = ({ data }: Props) => {
  ...
  const {
    ...
    queryFn: () => {
      return pksService.getBiayaSertifikasiByMahasiswa(idTrxMahasiswa!!);
    },
    staleTime: STALE_TIME,
    enabled: idTrxMahasiswa !== undefined,
  });

  const dataBiayaSertifikasi: ITrxBiayaSertifikasi[] = response?.data ?? [];

  if (isLoading) {
    return (
      <TabsContent value="biaya-sertifikasi" className="mt-0">
        ...
        <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
          <div className="text-red-600 mb-2">
            ...
            <p className="font-semibold text-lg">
              Gagal Memuat Data Biaya Sertifikasi
            </p>
  ...
  const tableRows = dataBiayaSertifikasi.map((item) => [
  ...
  return (
    <TabsContent value="biaya-sertifikasi" className="mt-0 p-1">
      <div className="space-y-4">
        <div>
          <SectionHeader
            title="Riwayat Biaya Sertifikasi"
            subtitle="Biaya Sertifikasi yang telah diajukan"
            Icon={History}
          />
          <ModernTable
            headers={["Jumlah", "Status Disetujui", "Status Transfer"]}
            rows={tableRows}
          />
        </div>
      </div>
