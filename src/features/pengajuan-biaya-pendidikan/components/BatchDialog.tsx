...
import type { BatchItem } from "@/types/biayaPendidikan";
...
export function generateBatchCode(selectedBatch: BatchItem[]): BatchCodeResult {
  ...
  const { tahun, semester, jenjang } = first;
  ...
  // 🔥 LOGIKA BARU SEMESTER
  const semesterNumber = parseInt(semester);

  if (isNaN(semesterNumber)) {
    return {
      code: null,
      message: "Batch tidak bisa digenerate: Semester tidak valid",
    };
  }

  const semesterCode = semesterNumber % 2 === 1 ? "41" : "42";

  const now = new Date();
  const day = String(now.getDate()).padStart(2, "0");
  const year = String(tahun).slice(-2);

  const batchCode = `${year}${semesterCode}${day}${jenjangCode}`;
...
export function BatchDialog({
  ...
  const mutation = useMutation({
    mutationFn: (data: { kode_batch: string; ids: string }) =>
      biayaPendidikanService.batchBiayaPendidikan(data.kode_batch, data.ids),
    onSuccess: (res) => {
      if (res.success) {
        ...
        queryClient.invalidateQueries({
          queryKey: ["log-pengajuan", "biaya-pendidikan"],
          exact: false,
        });
        onOpenChange(false);
        resetSelectedBatch();
    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-h-[90vh] overflow-y-auto font-inter">
          <DialogHeader>
            <DialogTitle>Batch Pengajuan Biaya Pendidikan</DialogTitle>
            <DialogDescription>
