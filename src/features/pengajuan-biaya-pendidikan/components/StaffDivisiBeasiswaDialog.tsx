...
const StaffDivisiBeasiswaDialog = ({
  ...
  const {
    ...
    queryKey: ["biaya-semester-lalu", dataBiayaPendidikan?.id],
    retry: false,
    refetchOnWindowFocus: false,
    queryFn: () => {
      return biayaPendidikanService.getTagihanSemesterLalu(
        dataBiayaPendidikan?.id!!,
      );
    },
    enabled: !!dataBiayaPendidikan?.id,
  ...
  useEffect(() => {
    reset({
      total_pagu: dataBiayaPendidikan?.biaya_pendidikan || 0,
      jumlah_biaya_pendidikan: dataBiayaPendidikan?.jumlah || 0,
      tagihan_semester_ini: dataBiayaPendidikan?.jumlah || 0,
      tagihan_semester_lalu: tagihanSemesterLalu,
    });
  }, [dataBiayaPendidikan, tagihanSemesterLalu, reset]);

  // ✅ PERBAIKAN: Watch dengan sintaks yang benar
  const total_pagu = Number(watch("total_pagu")) || 0;
  const jumlah_biaya_pendidikan = Number(watch("jumlah_biaya_pendidikan")) || 0;
  const tagihan_semester_lalu = Number(watch("tagihan_semester_lalu")) || 0;
  const tagihan_semester_ini = Number(watch("tagihan_semester_ini")) || 0;
  const tagihan_sd_semester_ini = Number(watch("tagihan_sd_semester_ini")) || 0;
  ...
  const mutation = useMutation({
    mutationFn: (data: StaffBeasiswaFormData) =>
      biayaPendidikanService.staffBeasiswa(dataBiayaPendidikan?.id!!, data),
    onSuccess: (res) => {
      if (res.success) {
