...
import { Calculator, Download, FileText, Wallet } from "lucide-react";
...
import { pksEditSchema, type PksEditFormData } from "@/types/pks";
...
const KeuanganEditPage = () => {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    ...
    queryFn: () => pksService.getDetailPksById(Number(idTrxPks!!)),
    enabled: !!idTrxPks,
    retry: false,
  ...
  const form = useForm<PksEditFormData>({
    resolver: zodResolver(pksEditSchema),
    defaultValues: {
      jumlah_mahasiswa: 0,
      is_swakelola: "N",
    },
  ...
  const idPksReferensi = watch("pks_referensi");

  // Jika menggunakan referensi, pagu biaya ambil dari referensi
  const {
    data: detailPksReferensiData,
    isLoading: detailPksReferensiIsLoading,
  } = useQuery({
    queryKey: ["pks-referensi", idPksReferensi],
    queryFn: () => pksService.getDetailPksById(parseInt(idPksReferensi!!)),
    retry: false,
    refetchOnWindowFocus: false,
    staleTime: STALE_TIME,
    enabled: idPksReferensi != undefined,
  });

  useEffect(() => {
    const detail = detailPksReferensiData?.data;
    if (!detail) return;

    setValue("jumlah_mahasiswa", detail.jumlah_mahasiswa || 0);
    setValue("biaya_hidup", detail.biaya_hidup || 0);
    setValue("biaya_pendidikan", detail.biaya_pendidikan || 0);
    setValue("biaya_buku", detail.biaya_buku || 0);
    setValue("biaya_transportasi", detail.biaya_transportasi || 0);
    setValue(
      "biaya_sertifikasi_kompetensi",
      detail.biaya_sertifikasi_kompetensi || 0,
    );
  }, [detailPksReferensiData, setValue]);
  ...
  const mutation = useMutation({
    mutationFn: (data: PksEditFormData) => pksService.editPks(idTrxPks!!, data),
    onSuccess: (res) => {
      if (res.success) {
  ...
  const onSubmit = (data: PksEditFormData) => {
    ...
    <>
      <CustBreadcrumb
        items={[
          {
            name: "Keuangan",
            url: "/database/keuangan",
          },
          { name: "Ubah Keuangan" },
        ]}
      />

      <p className="text-xl font-semibold mt-4">Ubah Keuangan</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                    <div className="space-y-1">
                      <div className="w-full flex justify-between items-center">
                        ...
                        <a
                          onClick={() =>
                            window.open(detailData?.file_pks!!, "_blank")
                          }
                  ...
                  <div className="bg-gray-50 p-6 rounded-lg border border-gray-200">
                    <div className="w-full space-y-1">
                      ...
                      {errors.tahun && (
                        <p className="text-xs text-destructive">
                          {errors.tahun.message as string}
                        </p>
                      )}
                    </div>
                  </div>
      ...
      <LoadingDialog
        open={detailPksReferensiIsLoading}
        title="Mengambil data referensi PKS"
      />
    </>
  );
};

export default KeuanganEditPage;
