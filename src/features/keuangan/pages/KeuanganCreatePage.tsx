...
import { pksCreateSchema, type PksCreateFormData } from "@/types/pks";
...
const KeuanganCreatePage = () => {
  useRedirectIfHasNotAccess("C");
  const queryClient = useQueryClient();
  const navigate = useNavigate();

  const form = useForm<PksCreateFormData>({
    resolver: zodResolver(pksCreateSchema),
    defaultValues: {
      jumlah_mahasiswa: 0,
      is_swakelola: "N",
    },
  ...
  const mutation = useMutation({
    mutationFn: (data: PksCreateFormData) => pksService.createPks(data),
    onSuccess: (res) => {
      if (res.success) {
  ...
  const onSubmit = (data: PksCreateFormData) => {
  ...
  return (
    <>
      ...
      <p className="text-xl font-semibold mt-4">Tambah Keuangan</p>
...
export default KeuanganCreatePage;
