...
import {
  ...
  type ListBuktiPengeluaranWithMahasiswa,
...
import FormListBuktiPengeluaran from "./FormListBuktiPengeluaran";
...
const FormAjukan = () => {
  ...
  const totalBiayaTransportasi =
    (statistikMahasiswa?.total_mahasiswa_aktif ?? 0) *
    (selectedPks?.biaya_transportasi_per_tahap_per_mahasiswa ?? 0);

  const now = new Date();

  const tahap = watch("tahap");
  const bulan = now.toLocaleString("id-ID", { month: "long" });
  const tahun = now.getFullYear().toString();

  const { data: getLockDataResponse, isLoading: isGetLockDataLoading } =
    useQuery({
      queryKey: ["pks", "mahasiswa", "lock-data", selectedPksId, tahap, tahun],
      queryFn: () => {
        ...
        return biayaTransportasiService.getLockedDataPks(data);
      },
      enabled: !!selectedPksId && !!tahap && !!tahun,
      retry: false,
      refetchOnWindowFocus: false,
      staleTime: STALE_TIME,
    });
  ...
  useEffect(() => {
    ...
    setValue("jumlah_nominal", totalBiayaTransportasi);
  }, [
    selectedPks,
    statistikMahasiswa,
    tahap,
    tahun,
    totalBiayaTransportasi,
    setValue,
  ]);
  ...
  const tujuanTransferOptions = [
    {
      value: "Lembaga Pendidikan",
  ...
  const generateNominatifForForm = async () => {
    ...
    try {
      const blob = await biayaTransportasiService.generateNominatif(
  ...
  const downloadSptjm = async () => {
    try {
      ...
      const blob = await biayaTransportasiService.generateSptjm({
        id_trx_pks: parseInt(selectedPksId),
        nominal: 0,
  ...
  const downloadBuktiKwitansi = async () => {
    try {
      ...
      const blob = await biayaTransportasiService.generateBuktiKwitansi({
        id_trx_pks: parseInt(selectedPksId),
        nominal: 0,
        total_mahasiswa_aktif: statistikMahasiswa?.total_mahasiswa_aktif ?? 0,
        tahap: tahap,
  ...
  const {
    data: getListBuktiPengeluaranResponse,
    isLoading: isGetListBuktiPengeluaranLoading,
  } = useQuery({
    queryKey: ["get-list-bukti-pengeluaran", selectedPksId, tahap],
    queryFn: () => {
      return biayaTransportasiService.getListBuktiPengeluaran(
        selectedPksId,
        tahap,
      );
    },
    enabled: !!selectedPksId && !!tahap,
    retry: false,
    refetchOnWindowFocus: false,
    staleTime: STALE_TIME,
  });

  const listBuktiPengeluaran: ListBuktiPengeluaranWithMahasiswa[] =
    getListBuktiPengeluaranResponse?.data ?? [];
  ...
  return (
    <>
      ...
      {selectedPks && tahap && !sudahDiajukan && !isGetLockDataLoading && (
                ...
                <>
                  <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                      ...
                      <div className="flex items-center gap-4 p-4 rounded-lg bg-purple-50 dark:bg-purple-950/20 border border-purple-100 dark:border-purple-900">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-red-600">
                            (otomatis terisi dari akumulasi nominal mahasiswa)
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-6">
                      ...
                      {/* ================= List Bukti Pengeluaran ================= */}
                      {isGetListBuktiPengeluaranLoading ? (
                        ...
                        <>
                          ...
                          <CustSelect
                            ...
                            options={tujuanTransferOptions}
                            placeholder="Pilih tujuan transfer"
                            error={errors.tujuan_transfer}
                          />
                          <FormListBuktiPengeluaran
                            data={listBuktiPengeluaran}
                          />
                        </>
                      )}
                    </div>
          ...
          <CardContent className="flex items-start gap-4 py-4">
            ...
            <div className="flex-1">
              <p className="font-semibold text-green-700 dark:text-green-300">
                Data Biaya Transportasi Sudah Diajukan
              </p>
              <p className="text-sm text-muted-foreground mt-1">
                Pengajuan biaya transportasi untuk PKS ini pada periode{" "}
