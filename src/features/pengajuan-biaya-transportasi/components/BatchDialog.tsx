...
import type { BatchItem } from "@/types/biayaTransportasi";
...
export function generateBatchCode(selectedBatch: BatchItem[]): BatchCodeResult {
  ...
  const { tahun, tahap, jenjang } = first;

  const allSame = selectedBatch.every(
    (item) =>
      item.tahun === tahun && item.tahap === tahap && item.jenjang === jenjang,
  );
  ...
  const tahapCode = tahap == "Kedatangan" ? "88" : "99";

  const now = new Date();
  const day = String(now.getDate()).padStart(2, "0");
  const year = String(tahun).slice(-2);

  const batchCode = `${year}${tahapCode}${day}${jenjangCode}`;
...
export function BatchDialog({
  ...
  const mutation = useMutation({
    mutationFn: (data: { kode_batch: string; ids: string }) =>
      biayaTransportasiService.batchBiayaTransportasi(
        data.kode_batch,
        data.ids,
      ),
    onSuccess: (res) => {
      if (res.success) {
        ...
        queryClient.invalidateQueries({
          queryKey: ["log-pengajuan", "biaya-transportasi"],
          exact: false,
        });
        onOpenChange(false);
        resetSelectedBatch();
    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-h-[90vh] overflow-y-auto font-inter">
          <DialogHeader>
            <DialogTitle>Batch Pengajuan Biaya Transportasi</DialogTitle>
            <DialogDescription>
