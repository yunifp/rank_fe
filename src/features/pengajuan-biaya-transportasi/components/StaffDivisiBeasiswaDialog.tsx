...
const StaffDivisiBeasiswaDialog = ({
  ...
  const {
    ...
    queryKey: ["biaya-tahap-lalu", dataBiayaTransportasi?.id],
    retry: false,
    refetchOnWindowFocus: false,
    queryFn: () => {
      return biayaTransportasiService.getTagihanTahapLalu(
        dataBiayaTransportasi?.id!!,
      );
    },
    enabled: !!dataBiayaTransportasi?.id,
  ...
  useEffect(() => {
    reset({
      total_pagu: dataBiayaTransportasi?.jumlah || 0,
      tagihan_tahap_lalu: tagihanTahapLalu,
    });
  }, [dataBiayaTransportasi, tagihanTahapLalu, reset]);
  ...
  const mutation = useMutation({
    mutationFn: (data: StaffBeasiswaFormData) =>
      biayaTransportasiService.staffBeasiswa(dataBiayaTransportasi?.id!!, data),
    onSuccess: (res) => {
      if (res.success) {
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 text-sm">
              ...
              <div>
                ...
                <p className="font-medium">
                  Tahap {dataBiayaTransportasi?.tahap}
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Button
                  ...
                  variant={"outline"}
                  onClick={() => {
                    window.open(dataBiayaTransportasi?.file_sptjm!!, "_blank");
                  }}
                >
                  <Download className="w-4 h-4 mr-1" />
                  Unduh
