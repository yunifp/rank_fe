...
const VerifikatorPjkDialog = ({ open, setOpen, dataBiayaHidup }: Props) => {
  ...
  const handleDownloadRab = async () => {
    try {
      ...
      const blob = await biayaHidupService.generateRab(dataBiayaHidup?.id!!);
  ...
  const mutation = useMutation({
    mutationFn: (data: VerifikatorPjkFormData) =>
      biayaHidupService.verifikatorPjk(dataBiayaHidup?.id!!, data),
    onSuccess: (res) => {
      if (res.success) {
