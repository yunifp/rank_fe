...
export function generateBatchCode(selectedBatch: BatchItem[]): BatchCodeResult {
  ...
  const { tahun, bulan, jenjang } = first;

  const allSame = selectedBatch.every(
    (item) =>
      item.tahun === tahun && item.bulan === bulan && item.jenjang === jenjang,
  );
  ...
  if (!allSame) {
    return {
      code: null,
      message:
        "Batch tidak bisa digenerate: Tahun, Bulan, atau Jenjang berbeda",
    };
  }

  if (!tahun || !bulan || !jenjang) {
    return {
      code: null,
  ...
  const bulanCode = bulanMap[bulan];
  if (!bulanCode) {
    return {
      code: null,
      message: "Batch tidak bisa digenerate: Bulan tidak valid",
    };
  }
  ...
  const batchCode = `${year}${bulanCode}${day}${jenjangCode}`;
  return {
    code: batchCode,
...
export function BatchDialog({
  ...
  const mutation = useMutation({
    mutationFn: (data: { kode_batch: string; ids: string }) =>
      biayaHidupService.batchBiayaHidup(data.kode_batch, data.ids),
    onSuccess: (res) => {
      if (res.success) {
        queryClient.invalidateQueries({ queryKey: ["pengajuan-biaya-hidup"] });
        queryClient.invalidateQueries({
          queryKey: ["log-pengajuan", "biaya-hidup"],
          exact: false,
        });
        onOpenChange(false);
        resetSelectedBatch();
    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-h-[90vh] overflow-y-auto font-inter">
          <DialogHeader>
            <DialogTitle>Batch Pengajuan Biaya Hidup</DialogTitle>
            <DialogDescription>
