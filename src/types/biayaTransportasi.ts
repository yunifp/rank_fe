...
export interface PaginatedBiayaTransportasiPksResponse {
  result: ITrxBiayaTransportasiPksWithPks[];
  total: number;
  current_page: number;
  total_pages: number;
}

export const biayaTransportasiPksCreateSchema = z.object({
  tahap: z
    .string({
  ...
  jumlah: z.coerce
    .number({
      required_error: "Jumlah biaya transportasi wajib diisi",
      invalid_type_error: "Jumlah harus berupa angka",
    })
    .min(1, "Jumlah minimal Rp 1"),
});

export type BiayaTransportasiPksCreateFormData = z.infer<
  typeof biayaTransportasiPksCreateSchema
>;

export interface ITrxBiayaTransportasiPks {
  id: number;
...
export interface ITrxBiayaTransportasiPksWithPks extends ITrxBiayaTransportasiPks {
...
export const ajukanKeVerifikatorSchema = z.object({
  ...
  tahap: z.string().min(1, "Tahap wajib dipilih"),
...
export interface PaginatedBatchBiayaTransportasiPksResponse {
  result: IBatchBiayaTransportasiPks[];
  total: number;
  current_page: number;
  total_pages: number;
}

export interface IBatchBiayaTransportasiPks {
  id: number;
  nama: string;
  tahap: string;
  tahun: string;
...
export interface ListBuktiPengeluaranWithMahasiswa extends IMahasiswaPks {
